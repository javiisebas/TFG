...
'''
 This library creates function that displays graphics of the studied parameters
'''
...
def Plot_Separeted_Main(save):
	...
	# Dictionary that will be usefull to create the graphics
	plot_dic = {
	...
	# In case we want to save the graph in a image format
	if save:
		plt.savefig(os.getcwd() + '/Images/Separeted_Parameters_Shells.png')
...
def Plot_Compound_Main(save):
	...
	# In case we want to save the graph in a image format
	if save:
		plt.savefig(os.getcwd() + '/Images/Compound_Parameters_Shells.png')
...
def Plot_Luminosity_Gradient(save):
	'''
	...
	# In case we want to save the graph in a image format
	if save:
		plt.savefig(os.getcwd() + '/Images/Luminosity_Gradient.png')
...
def Plot_Compound(save):
	...
	# We add a super title for both graphics
	plt.suptitle('\nRELATIVE ERROR STUDY\n', fontsize = 24, fontweight='bold')

	# In case we want to save the graph in a image format
	if save:
		plt.savefig(os.getcwd() + '/Images/Compound_Relative_Error.png')
...
def Plot_Radius_Luminosity(save):
	'''
	...
	# In case we want to save the graph in a image format
	if save:
		plt.savefig(os.getcwd() + '/Images/Radius_Luminosity_Error.png')
...
def Plot_Temperature(save):
	...
	# In case we want to save the graph in a image format
	if save:
		plt.savefig(os.getcwd() + '/Images/Temperature_Error.png')
